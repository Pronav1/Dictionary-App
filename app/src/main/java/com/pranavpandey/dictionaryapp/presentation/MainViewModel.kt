    ...
    fun onEvent(mainEvent: MainUiEvents){
        when(mainEvent){
            ...
            is MainUiEvents.OnSearchWordChange -> {
                _mainState.update {
                    it.copy(searchWord = mainEvent.newWord.lowercase())
                }
            }
        }
...
/*private fun loadWordResult() {
    viewModelScope.launch {
        dictionaryRepository.getWordResult(mainState.value.searchWord)
            .collect { result ->
                when (result) {
                    is Result.Error -> {
                        Log.e("loadWordResult", "Error loading word: ${result.exception}")
                        _mainState.update {
                    ...
                    is Result.Success -> {
                        _mainState.update {
                            it.copy(wordItem = result.data ?: WordItem())
                        }
                    }
                }
            }
